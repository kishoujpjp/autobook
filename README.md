...
給 5 歲小朋友的中文認字／英語啟蒙 PWA，主要在 iPad 13 吋使用。目前版本 **v1.18.1**。
...
- **iOS 原生殼（Capacitor，v1.18.1 起）**：`com.kishou.autobook`，免費開發者帳號簽名直接裝在 iPad 上。
...
### 字表
- （v1.18.1）「補齊發音」按鈕已移除（單字交給 Gemini TTS 容易唸錯，暫不使用）；設定頁的「清除語音快取」「載入示範字表」改為先跳確認框。
